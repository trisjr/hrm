...
function TeamIDPDetailPage() {
  ...
  const { data: idpData, isLoading } = useQuery({
    ...
    queryFn: () =>
      getIDPByIdFn({
        data: { token: token!, data: { idpId: Number(idpId) } },
      } as any),
  })
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-4">
          ...
          <Badge
            ...
            variant={idp.status === 'IN_PROGRESS' ? 'default' : 'secondary'}
          >
              ...
              {idp.activities.map((act: any) => (
                <TableRow key={act.id}>
                  <TableCell className="font-medium">
                    {act.competency?.name || 'General'}
                  </TableCell>
                  <TableCell>
                    <div className="line-clamp-2" title={act.description}>
                      {act.description}
                    </div>
                  </TableCell>
                  <TableCell>
