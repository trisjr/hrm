/**
 * Dashboard Homepage
 * Displays personalized dashboard based on a user role
 */
import { createFileRoute } from '@tanstack/react-router'
import { useSuspenseQuery } from '@tanstack/react-query'
import { getDashboardStatsFn } from '@/server/dashboard.server'
import { useAuthStore } from '@/store/auth.store'
import { Award, Building2, CalendarDays, Home, Users } from 'lucide-react'
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="grid gap-6 md:grid-cols-3 lg:grid-cols-3">
          ...
          {isLeaderOrAdmin && teamStats && (
            <div className="grid gap-6 md:grid-cols-2">
                      ...
                      {teamStats.absentToday.map((req: any) => (
                        ...
                        >
                          ...
                          <Badge variant="outline" className="text-[10px]">
                            {format(new Date(req.startDate), 'HH:mm')} -{' '}
                            {format(new Date(req.endDate), 'HH:mm')}
                          </Badge>
                        </div>
                      ))}
                    </div>
              ...
              <RequestsWidget
                ...
                requests={teamStats.pendingApprovals as any}
                showUser={true}
        ...
        <div className="md:col-span-1 lg:col-span-1 space-y-6">
              ...
              {upcomingHolidays.length === 0 ? (
                ...
                <div className="space-y-4">
                  {upcomingHolidays.map((h: any) => (
                    <div
                      key={h.id}
                      className="flex flex-col bg-accent/50 p-3 rounded-md"
                    >
          ...
          {/* Quick Tip (Static) */}
          <Card className="bg-linear-to-br from-indigo-50 to-purple-50 border-none shadow-sm">
