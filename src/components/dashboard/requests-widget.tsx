...
export function RequestsWidget({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'APPROVED':
        return 'default' // shadcn default is effectively black/primary, usually stick directly to badge variant
      case 'REJECTED':
        return 'destructive'
      default:
        return 'secondary' // Yellow/Orange often mapped to secondary or outline
    }
  }
              ...
              >
                <div className="space-y-1">
                  <div className="font-medium text-sm flex items-center gap-2">
                    ...
                    {showUser && (
                      <span className="text-primary">{req.user?.fullName}</span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant={getStatusColor(req.status!) as any}
